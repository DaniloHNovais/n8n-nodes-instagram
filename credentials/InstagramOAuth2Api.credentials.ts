...
export class InstagramOAuth2Api implements ICredentialType {
	...
	properties: INodeProperties[] = [
		...
		},
		{
			...
			type: 'notice',
			default: '',
			description: 'After connecting your account, your Instagram Business Account details (username, ID, profile) will be automatically available. Your workflow runtime will automatically exchange the OAuth token for a long-lived token (60 days) and refresh it when needed.',
		},
		{
		...
		// Hidden compatibility fields kept for users upgrading from earlier versions.
		{
	...
	/**
	 * Compatibility fallback for n8n code paths that may still invoke preAuthentication.
	 *
	 * The node now performs Instagram-specific token exchange and refresh during request execution,
	 * because n8n's built-in OAuth2 request helper does not run preAuthentication for credentials
	 * extending oAuth2Api.
	 */
	async preAuthentication(
		...
		const oauthTokenData = credentials.oauthTokenData as
			...
			| undefined;
		const shortLivedToken = oauthTokenData?.access_token ?? oauthTokenData?.accessToken;
